...
def bas(touche):
  ...
  if (pusher.get('Control_R',0) or pusher.get('Control_L',0)) and pusher.get('s'):
    try:
      ...
      pass
  elif (pusher.get('Control_R',0) or pusher.get('Control_L',0)) and pusher.get('o'):
    try:
      file = askopenfilename(filetypes=('All files', '*.*'))
      T.insert('1.0', file.readlines())
      f.title(file.name)
    except:
      pass
  pusher={}
...
coul('r')
T.pack()
tk.Label(f,text='CTRL-S: sauvegarder Click-molette: changer la couleur',fg='white',bg='black').pack()
